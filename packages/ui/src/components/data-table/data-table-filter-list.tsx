...
import { DataTableRangeFilter } from "@adaptyv-coordination/ui/components/data-table/data-table-range-filter";
...
import {
  ...
  FacetedList,
  FacetedTrigger,
} from "@adaptyv-coordination/ui/components/ui/faceted";
...
import {
  ...
  SortableItemHandle,
  SortableOverlay,
} from "@adaptyv-coordination/ui/components/ui/sortable";
import { dataTableConfig } from "@adaptyv-coordination/config/data-table";
import { useDebouncedCallback } from "@adaptyv-coordination/ui/hooks/use-debounced-callback";
import { getDefaultFilterOperator, getFilterOperators } from "@adaptyv-coordination/ui/lib/data-table";
import { formatDate } from "@adaptyv-coordination/ui/lib/format";
import { generateId } from "@adaptyv-coordination/ui/lib/id";
import { getFiltersStateParser } from "@adaptyv-coordination/ui/lib/parsers";
import { cn } from "@adaptyv-coordination/ui/lib/utils";
import type {
  ExtendedColumnFilter,
  FilterOperator,
  JoinOperator,
} from "@adaptyv-coordination/types/data-table";
  ...
  return (
    ...
    >
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger render={<Button variant="outline" size="sm" className="font-normal" onKeyDown={onTriggerKeyDown} disabled={disabled} />}><ListFilter className="text-muted-foreground" />Filter
                          {filters.length > 0 && (
                            <Badge
                              variant="secondary"
...
function DataTableFilterItem<TData>({
  ...
  return (
    <SortableItem value={filter.filterId} render={<div role="listitem" id={filterItemId} tabIndex={-1} className="flex items-center gap-2" onKeyDown={onItemKeyDown} />}><div className="min-w-[72px] text-center">
                ...
                <PopoverTrigger render={<Button aria-controls={fieldListboxId} variant="outline" size="sm" className="w-32 justify-between rounded font-normal" />}><span className="truncate">
    ...
    case "multiSelect": {
      ...
      return (
        <Faceted
          ...
          multiple={multiple}
        >
          <FacetedTrigger render={<Button id={inputId} aria-controls={inputListboxId} aria-label={`${columnMeta?.label} filter value${multiple ? "s" : ""}`} variant="outline" size="sm" className="w-full rounded font-normal" />}><FacetedBadgeList
    ...
    case "dateRange": {
      ...
      return (
        <Popover open={showValueSelector} onOpenChange={setShowValueSelector}>
          <PopoverTrigger render={<Button id={inputId} aria-controls={inputListboxId} aria-label={`${columnMeta?.label} date filter`} variant="outline" size="sm" className={cn(
